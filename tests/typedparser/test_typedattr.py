...
from typedparser._typedattr import NamedTupleMixin
...
def test_readme_content():
    ...
    assert (
        str(attrs_from_dict(CfgNested, {"sub_cfg": {"foo": 1, "bar": 2}}))
...
def test_namedtuplemixin():
    ...
    @define
    class DefinedNamedTuple(NamedTupleMixin):
        foo: int
        bar: Optional[int] = None

    no = DefinedNormal(12)
    tu = DefinedNamedTuple(12)
    print(tu)
